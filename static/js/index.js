...
window.onload = function () {
    ...
    Promise.all([
        ...
        const footerContainer = document.querySelector('footer .footer-container p');
        if (footerContainer) {
            footerContainer.innerHTML = `&copy; ${footerYear} Library Catalog. All rights reserved.`;
        }
    }).catch(error => {
        console.error('Error fetching or parsing XML files:', error);
        displayError('An error occurred while loading the book data. Please try again later.');
    });
    ...
    document.getElementById('borrowForm').addEventListener('submit', function (event) {
        ...
        if (row) {
            ...
            if (currentState === 'Borrowed') {
                displayError('This book is already borrowed.');
                return;
            }
            ...
            displayError('No book found with that ID.');
        }
    });
    ...
    document.getElementById('returnForm').addEventListener('submit', function (event) {
        ...
        if (row) {
            const currentState = row.cells[8].textContent;
            if (currentState !== 'Borrowed') {
                displayError('This book is not currently borrowed.');
                return;
            }
